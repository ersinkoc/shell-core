...
exports.HotReloader = exports.FileWatcherImpl = void 0;
 ...
 */
class FileWatcherImpl extends events_1.EventEmitter {
    ...
    async setupWatchers(paths) {
        try {
            ...
            this.emit('error', new errors_js_1.ShellError(`Failed to setup file watchers: ${error.message}`, 'INVALID_OPERATION', 'watch.setup', undefined, undefined, undefined, { paths, error }, false));
        }
    }
    async addPath(watchPath) {
        const fullPath = (0, path_1.resolve)(this.options.cwd, watchPath);
    ...
    async watchDirectory(dirPath) {
        ...
        const watcher = (0, fs_1.watch)(dirPath, {
        ...
        watcher.on('change', (eventType, filename) => {
            if (filename) {
                const fullPath = (0, path_1.join)(dirPath, String(filename));
                this.handleFileEvent(eventType, fullPath);
            }
        });
        watcher.on('error', (error) => {
            this.emit('error', new errors_js_1.ShellError(`Directory watcher error: ${error.message}`, 'INVALID_OPERATION', 'watch.directory', dirPath, undefined, undefined, { dirPath, error }, true));
        });
            ...
            try {
                ...
                for (const entry of entries) {
                    if (entry.isDirectory()) {
                        const subDirPath = (0, path_1.join)(dirPath, entry.name);
    ...
    async watchFile(filePath) {
        ...
        watcher.on('error', (error) => {
            this.emit('error', new errors_js_1.ShellError(`File watcher error: ${error.message}`, 'INVALID_OPERATION', 'watch.file', filePath, undefined, undefined, { filePath, error }, true));
        });
    ...
    handleWriteFinish(path, eventType, stats) {
        ...
        const timer = setTimeout(async () => {
            try {
                const currentStats = await (0, promises_1.stat)(path);
    ...
    shouldIgnore(path) {
        ...
        const relativePath = (0, path_1.relative)(this.options.cwd, path);
    ...
    add(paths) {
        if (this.closed) {
            throw new errors_js_1.ShellError('Cannot add paths to closed watcher', 'INVALID_OPERATION', 'watch.add', undefined, undefined, undefined, undefined, false);
        }
    ...
    unwatch(paths) {
        ...
        for (const path of pathsArray) {
            const fullPath = (0, path_1.resolve)(this.options.cwd, path);
    ...
    getWatched() {
        ...
        for (const path of this.watchedPaths) {
            const dir = (0, path_1.resolve)(path, '..');
            if (!watched[dir]) {
                watched[dir] = [];
            }
            watched[dir].push((0, path_1.relative)(dir, path));
        }
        return watched;
    }
}
 ...
 */
function createHotReloader(options) {
