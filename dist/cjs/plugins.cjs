...
exports.GitPlugin = exports.BasePlugin = exports.PluginManager = void 0;
...
class PluginManager {
    ...
    use(plugin) {
        if (!plugin.name || !plugin.version) {
            throw new errors_js_1.ShellError('Plugin must have name and version properties', 'PLUGIN_ERROR', 'use');
        }
        if (this.plugins.has(plugin.name)) {
            throw new errors_js_1.ShellError(`Plugin '${plugin.name}' is already installed`, 'PLUGIN_ERROR', 'use');
        }
        try {
            // Install plugin
            ...
            throw new errors_js_1.ShellError(`Failed to install plugin '${plugin.name}': ${error instanceof Error ? error.message : String(error)}`, 'PLUGIN_ERROR', 'use', undefined, undefined, undefined, error);
        }
    }
    unuse(pluginName) {
        const plugin = this.plugins.get(pluginName);
        if (!plugin) {
            throw new errors_js_1.ShellError(`Plugin '${pluginName}' is not installed`, 'PLUGIN_ERROR', 'unuse');
        }
        try {
            // Unregister commands
            ...
            throw new errors_js_1.ShellError(`Failed to uninstall plugin '${pluginName}': ${error instanceof Error ? error.message : String(error)}`, 'PLUGIN_ERROR', 'unuse', undefined, undefined, undefined, error);
        }
    }
    getPlugin(name) {
    ...
    async executeCommand(name, ...args) {
        ...
        if (!handler) {
            throw new errors_js_1.ShellError(`Command '${name}' not found`, 'PLUGIN_ERROR', 'executeCommand');
        }
        try {
            return await handler(...args);
        }
        catch (error) {
            throw new errors_js_1.ShellError(`Command '${name}' failed: ${error instanceof Error ? error.message : String(error)}`, 'PLUGIN_ERROR', 'executeCommand', undefined, undefined, undefined, error);
        }
    }
    hasFilter(name) {
    ...
    applyFilter(name, input) {
        ...
        if (!handler) {
            throw new errors_js_1.ShellError(`Filter '${name}' not found`, 'PLUGIN_ERROR', 'applyFilter');
        }
        try {
            return handler(input);
        }
        catch (error) {
            throw new errors_js_1.ShellError(`Filter '${name}' failed: ${error instanceof Error ? error.message : String(error)}`, 'PLUGIN_ERROR', 'applyFilter', undefined, undefined, undefined, error);
        }
    }
    hasTransformer(name) {
    ...
    getTransformer(name, ...args) {
        ...
        if (!handler) {
            throw new errors_js_1.ShellError(`Transformer '${name}' not found`, 'PLUGIN_ERROR', 'getTransformer');
        }
        ...
        if (args.length > 0) {
            ...
            throw new errors_js_1.ShellError(`Transformer '${name}' with arguments did not return a function`, 'PLUGIN_ERROR', 'getTransformer');
        }
        // Otherwise, return the transformer directly if it's a function
        if (typeof handler === 'function') {
            return handler;
        }
        throw new errors_js_1.ShellError(`Transformer '${name}' is not a function`, 'PLUGIN_ERROR', 'getTransformer');
    }
    applyTransformer(name, input) {
        const handler = this.transformers.get(name);
        if (!handler) {
            throw new errors_js_1.ShellError(`Transformer '${name}' not found`, 'PLUGIN_ERROR', 'applyTransformer');
        }
        try {
            return handler(input);
        }
        catch (error) {
            throw new errors_js_1.ShellError(`Transformer '${name}' failed: ${error instanceof Error ? error.message : String(error)}`, 'PLUGIN_ERROR', 'applyTransformer', undefined, undefined, undefined, error);
        }
    }
    getCommands() {
